package com.suntelecoms.library_mifare;
/*
...
import com.suntelecoms.library_mifare.Common.Operations;
...
public class MCReader {
    ...
    private static final String LOG_TAG = com.suntelecoms.library_mifare.MCReader.class.getSimpleName();
    /**
    ...
    /**
     * Get new instance of {@link com.suntelecoms.library_mifare.MCReader}.
     * If the tag is "null" or if it is not a MIFARE Classic tag, "null"
     * will be returned.
     * @param tag The tag to operate on.
     * @return {@link com.suntelecoms.library_mifare.MCReader} object or "null" if tag is "null" or tag is
     * not MIFARE Classic.
     */
    public static com.suntelecoms.library_mifare.MCReader get(Tag tag) {
        com.suntelecoms.library_mifare.MCReader mcr = null;
        if (tag != null) {
            try {
                mcr = new com.suntelecoms.library_mifare.MCReader(tag);
    ...
    public void setKeyFile(File[] keyFiles, List<String> etabClientKeysList) {
        HashSet<byte[]> keys = new HashSet<byte[]>();

        List<String> secretKeysList = etabClientKeysList;
        if (secretKeysList != null && secretKeysList.size() > 0 ) {

            for (String secretKey : secretKeysList) {
                // Log.i("setKeyF. MCReader","Current secretKey :  "+secretKey);
                keys.add(Common.hexStringToByteArray(secretKey));
            }

            //TODO La clé secréte 3 ne doit être pris en compte
            //keys.add(Common.hexStringToByteArray(secretKeysList.get(1)));
            //keys.add(Common.hexStringToByteArray(secretKeysList.get(2)));
        ...
        for (File file : keyFiles) {
            ...
            if (lines != null) {
                for (String line : lines) {
                    if (!line.equals("") && line.length() == 12
                            && line.matches("[0-9A-Fa-f]+")) {
                        //Log.i("MCReader : ","###### setKeyFile ##### "+line);
                        keys.add(Common.hexStringToByteArray(line));
                    }
                }
            }
        ...
        Log.i("setKeyFile Size >> ", String.valueOf(keys.size()));
        if (keys.size() > 0) {
