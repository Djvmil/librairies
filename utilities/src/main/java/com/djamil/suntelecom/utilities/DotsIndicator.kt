...
class DotsIndicator(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs) {
    ...
    init {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.Utils_DotsIndicator)
        ...
        try {
            intrinsicWidth = ta.getDimensionPixelSize(R.styleable.Utils_DotsIndicator_dot_width, -1)
            intrinsicHeight = ta.getDimensionPixelSize(R.styleable.Utils_DotsIndicator_dot_height, -1)
            intrinsicMargin = ta.getDimensionPixelSize(R.styleable.Utils_DotsIndicator_dot_margin, -1)
            intrinsicOrientation = ta.getInt(R.styleable.Utils_DotsIndicator_theme5_dots_orientation, -1)
            intrinsicGravity = ta.getInt(R.styleable.Utils_DotsIndicator_theme5_dots_orientation, -1)

            this.animatorResId = ta.getResourceId(R.styleable.Utils_DotsIndicator_dots_animator, R.anim.utils_scale_with_alpha)
            this.animatorReverseResId = ta.getResourceId(R.styleable.Utils_DotsIndicator_dots_animator_reverse, 0)
            this.backgroundResId = ta.getResourceId(R.styleable.Utils_DotsIndicator_dot_drawable, R.drawable.utils_bg_black_dot)
            this.unselectedBackgroundId = ta.getResourceId(R.styleable.Utils_DotsIndicator_dot_drawable_unselected, this.backgroundResId)
            this.dotTint = ta.getColor(R.styleable.Utils_DotsIndicator_dot_tint, 0)
        } finally {
            ta.recycle()
        }
        ...
        indicatorBackgroundResId = if (this.backgroundResId == 0) R.drawable.utils_bg_black_dot else this.backgroundResId
