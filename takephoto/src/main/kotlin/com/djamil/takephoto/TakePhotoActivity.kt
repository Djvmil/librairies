...
import com.djamil.takephoto.provider.CameraProvider
import com.djamil.takephoto.provider.CompressionProvider
import com.djamil.takephoto.provider.CropProvider
import com.djamil.takephoto.provider.GalleryProvider
import java.io.File
...
class TakePhotoActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        internal fun getCancelledIntent(context: Context): Intent {
            ...
            intent.putExtra(TakePhoto.EXTRA_ERROR, message)
            return intent
        }
    }
    ...
    private fun loadBundle(savedInstanceState: Bundle?) {
        ...
        val provider: ImageProvider? =
            intent?.getSerializableExtra(TakePhoto.EXTRA_IMAGE_PROVIDER) as ImageProvider?
    ...
    private fun setResult(file: File) {
        ...
        intent.putExtra(TakePhoto.EXTRA_FILE_PATH, file.absolutePath)
    ...
    fun setError(message: String) {
        val intent = Intent()
        intent.putExtra(TakePhoto.EXTRA_ERROR, message)
        setResult(TakePhoto.RESULT_ERROR, intent)
        finish()
    }
}
