package com.djamil.takephoto.provider
...
import com.djamil.takephoto.TakePhoto
import com.djamil.takephoto.TakePhotoActivity
import com.djamil.takephoto.util.ExifDataCopier
import com.djamil.takephoto.util.FileUriUtils
import com.djamil.takephoto.util.FileUtil
import com.djamil.takephoto.util.ImageUtil
import java.io.File

/**
 * @ref https://github.com/Dhaval2404/ImagePicker
 */
class CompressionProvider(activity: TakePhotoActivity) : BaseProvider(activity) {
    ...
    init {
        ...
        mMaxWidth = bundle.getInt(TakePhoto.EXTRA_MAX_WIDTH, 0)
        mMaxHeight = bundle.getInt(TakePhoto.EXTRA_MAX_HEIGHT, 0)

        // Get Maximum Allowed file size
        mMaxFileSize = bundle.getLong(TakePhoto.EXTRA_IMAGE_MAX_SIZE, 0)

        // Get File Directory
        val fileDir = bundle.getString(TakePhoto.EXTRA_SAVE_DIRECTORY)
        fileDir?.let {
            mFileDir = File(it)
        }
    }
        ...
        object : AsyncTask<File, Void, File>() {
            ...
            override fun onPostExecute(file: File?) {
                ...
                if (file != null) {
                    ...
                    // Post Error
                    setError(com.djamil.takephoto.R.string.error_failed_to_compress_image)
                }
            }
        }.execute(file)
    }
