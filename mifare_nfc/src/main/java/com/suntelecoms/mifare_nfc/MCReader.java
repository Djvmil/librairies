...
package com.suntelecoms.mifare_nfc;
...
import com.suntelecoms.mifare_nfc.Common.Operations;
...
public class MCReader {
    ...
    private static final String LOG_TAG = com.suntelecoms.mifare_nfc.MCReader.class.getSimpleName();
    /**
    ...
    /**
     * Get new instance of {@link com.suntelecoms.mifare_nfc.MCReader}.
     * If the tag is "null" or if it is not a MIFARE Classic tag, "null"
     * will be returned.
     * @param tag The tag to operate on.
     * @return {@link com.suntelecoms.mifare_nfc.MCReader} object or "null" if tag is "null" or tag is
     * not MIFARE Classic.
     */
    public static com.suntelecoms.mifare_nfc.MCReader get(Tag tag) {
        com.suntelecoms.mifare_nfc.MCReader mcr = null;
        if (tag != null) {
            try {
                mcr = new com.suntelecoms.mifare_nfc.MCReader(tag);
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(com.suntelecoms.mifare_nfc.Common.byte2HexString(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (noData) {
                ...
                if (!useAsKeyB) {
                    if (isKeyBReadable(com.suntelecoms.mifare_nfc.Common.hexStringToByteArray(
                            ret[last].substring(12, 20)))) {
                        ret[last] = com.suntelecoms.mifare_nfc.Common.byte2HexString(key)
                    ...
                    ret[last] = NO_KEY + ret[last].substring(12, 20)
                            + com.suntelecoms.mifare_nfc.Common.byte2HexString(key);
                }
            }
        }
        return ret;
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            boolean autoReconnect = com.suntelecoms.mifare_nfc.Common.getPreferences().getBoolean(
                    Preference.AutoReconnect.toString(), false);
            // Get retry authentication option.
            boolean retryAuth = com.suntelecoms.mifare_nfc.Common.getPreferences().getBoolean(
                    Preference.UseRetryAuthentication.toString(), false);
            int retryAuthCount = com.suntelecoms.mifare_nfc.Common.getPreferences().getInt(
     ...
     * ({@link com.suntelecoms.mifare_nfc.Common#getOperationInfoForBlock(byte, byte, byte,
     * com.suntelecoms.mifare_nfc.Common.Operations, boolean, boolean)}) of the
     * Access Conditions.
     ...
     * {@link com.suntelecoms.mifare_nfc.Activities.KeyMapCreator}.
    ...
    public HashMap<Integer, HashMap<Integer, Integer>> isWritableOnPositions(
        ...
        for (int i = 0; i < keyMap.size(); i++) {
            ...
            if (pos.containsKey(sector)) {
                ...
                byte[][] acMatrix = com.suntelecoms.mifare_nfc.Common.acBytesToACMatrix(ac);
                if (acMatrix == null) {
                    ret.put(sector, null);
                    continue;
                }
                boolean isKeyBReadable = com.suntelecoms.mifare_nfc.Common.isKeyBReadable(
                ...
                for (int block : pos.get(sector)) {
                    if ((block == 3 && sector <= 31)
                        ...
                        int acValue = com.suntelecoms.mifare_nfc.Common.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                Operations.WriteAC,
                        ...
                        // with the same key.)
                        int keyABValue = com.suntelecoms.mifare_nfc.Common.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                        ...
                        blockWithWriteInfo.put(
                                block, com.suntelecoms.mifare_nfc.Common.getOperationInfoForBlock(
    ...
    public boolean setKeyFile(File[] keyFiles, Context context) {
        ...
        for (File file : keyFiles) {
            String[] lines = com.suntelecoms.mifare_nfc.Common.readFileLineByLine(file, false, context);
            if (lines != null) {
                for (String line : lines) {
                    if (!line.equals("") && line.length() == 12
                            && line.matches("[0-9A-Fa-f]+")) {
                        try {
                            keys.add(com.suntelecoms.mifare_nfc.Common.hexStringToByteArray(line));
    ...
    private boolean authenticate(int sectorIndex, byte[] key,
        ...
        boolean retryAuth = com.suntelecoms.mifare_nfc.Common.getPreferences().getBoolean(
                Preference.UseRetryAuthentication.toString(), false);
        int retryCount = com.suntelecoms.mifare_nfc.Common.getPreferences().getInt(
                Preference.RetryAuthenticationCount.toString(), 1);
        boolean ret = false;
    ...
    public int getSectorCount() {
        boolean useCustomSectorCount = com.suntelecoms.mifare_nfc.Common.getPreferences().getBoolean(
                Preference.UseCustomSectorCount.toString(), false);
        if (useCustomSectorCount) {
            return com.suntelecoms.mifare_nfc.Common.getPreferences().getInt(
