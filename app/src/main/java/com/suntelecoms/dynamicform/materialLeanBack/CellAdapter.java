...
public class CellAdapter extends RecyclerView.Adapter {
    ...
    protected CellAdapter.HeightCalculatedCallback heightCalculatedCallback;

    public CellAdapter(int row, MaterialLeanBack.Adapter adapter, MaterialLeanBackSettings settings, OnItemClickListenerWrapper onItemClickListenerWrapper,  CellAdapter.HeightCalculatedCallback heightCalculatedCallback) {
        this.row = row;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int type) {
        ...
        switch (type) {
            case PLACEHOLDER_START:
                view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.mlb_placeholder, viewGroup, false);//ici
                return new PlaceHolderViewHolder(view, true, settings.paddingLeft);
            case PLACEHOLDER_END:
                view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.mlb_placeholder_end, viewGroup, false);//ici
                return new PlaceHolderViewHolder(view, true, settings.paddingRight);
            default:
                view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.mlb_cell, viewGroup, false);//ici
